...
def objective(trial):
    """
    Optuna が呼び出す目的関数。
    trial からハイパーパラメータをサンプリングして、
    run_training_diff_bce() に渡し、
    得られたバリデーションロスを返すわ。
    """
    # サンプリング例: 学習率
    ...
    # 好みに応じて weight_decay などもサンプリングできるわ
    weight_decay = trial.suggest_loguniform('weight_decay', 1e-6, 1e-3)

    # あと学習epoch数とか patience とかも trial.suggest で探ってもOK

    # run_training_diff_bce を実行
    best_val_loss = run_training_diff_bce(
        data_path=DATA_PATH,
        pred_path=PRED_PATH,
        # rank_col="着順", # 固定なら省略
        # pop_col="単勝",  # 固定なら省略
        ...
        lr=lr,
        num_epochs=20,       # epoch数は長すぎると探索に時間かかるので例として固定少なめ
        d_model=d_model,
        nhead=nhead,
        num_layers=num_layers,
        dropout=dropout,
...
def run_optuna():
    # Optuna の Study を作成して実行
    study = optuna.create_study(direction="minimize")  # バリデーションロスを最小化したい想定
    study.optimize(objective, n_trials=20)  # 試行回数を指定
